...
Guidelines:
- ThreePointsWeb default is wait-for-result: even with run_in_background: true, the Agent tool waits for completion before returning so the parent cannot continue without using the result.
...
- Use isolation: "worktree" to run the agent in an isolated git worktree (safe parallel file modifications).`,
    parameters: Type.Object({
      ...
      run_in_background: Type.Optional(
        Type.Boolean({
          description: "Set to true to allow concurrent background scheduling. ThreePointsWeb default still waits for completion before returning; async mode must be explicitly enabled in settings.",
        }),
      ),
